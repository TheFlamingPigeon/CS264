...
int main() {
  ...
  cly->name = "Bob";
  cly->age = 51;
  (*clz).name = "Mark";  // using a different way of accesing the struct fields dereferencing the struct pointer using *
  ...
  // create an additional extra struct node, which we will add to the link list.
  ...
  cout << endl << endl << "The second last call to the display function " << endl << endl;
  // display the list, which should now contain an extra node added at the end.
  display(clx);
  
  /*

  The code below, inside these comments, causes a segmentation fault when we try and pass a pointer with a null value to the 
  addToBack function in order to add a new node to the head of the list. 
  I don't know why this segmentation fault happens, 4th of December 2017

  cout << endl << endl << "The last call to the display function " << endl;
  cout << " We are trying to add a node to an empty list, and then print it out" << endl << endl;
  client * trial = NULL;  // This is the empty list (pointer)

  addToBack(trial,cl_u); // we are adding cl_u to end of another list.
...
void addToBack(client * head, client * addedBox) {
  ...
  if (head == NULL) { 
   	...
   	head = addedBox;  // in this case the new node is the first and only node on the list.
        cout << endl << " ---- " << head->name << " --- " << head->age << endl;
  } 
  ...
  } // end code to add a node to the list.
